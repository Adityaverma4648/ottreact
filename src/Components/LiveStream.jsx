...
return (
  <div className='w-full flex lg:flex-col flex-row justify-start items-center lg:overflow-y-scroll lg:overflow-x-hidden overflow-x-scroll overflow-y-hidden scroll-smooth' onMouseEnter={mouseEnter} onMouseLeave={mouseOut}  ref={slider} id='carousel2' >
      {props.children}
   </div>
)
}
...
const LiveStream = () => {
  ...
  const genreData = [
    {id : 0, title : "genre", fileUrl : categoryPic },
  ...
  const shows = [
    {id : 0, title : "type", fileUrl : categoryPic },
    {id : 1, title : "movie", fileUrl : categoryPic },
    {id : 2, title : "series", fileUrl : categoryPic },
    {id : 3, title : "tvShows", fileUrl : categoryPic },
    {id : 4, title : "anime", fileUrl : categoryPic },
  ];
  ...
  useEffect(()=>{
      if(genre !== "genre"){
        ...
        data.forEach((d)=>{
                ...
                var genreArray = [d.Genre];
                var bufferString = "";
                 for (let key in genreArray){
                    bufferString += (genreArray[key].toLocaleLowerCase());
                }
                if(bufferString.indexOf(genre) === 0 || bufferString.indexOf(genre) !== -1 ){
                  arr.push(d);
                }
  ...
  return (
    ...
    <div className="w-full flex flex-row justify-center items-center ">
         <div className="w-full lg:h-20 h-12 flex lg:flex-col justify-between items-center ">
            ...
            <div className='w-1/2 flex xl:flex-row lg:flex-col xl:justify-center justify-end items-center'>
              <select name="genre" id="genre" className='bg-transparent select-none outline-none border-0 text-red-500 p-2' onChange={(e)=>{filter(e)}}>
                  {genreData?.map((d)=>{
                    return <option key={d.id} className='bg-transparent text-red-500' value={d.title}>{d.title}</option>
                  })}
              </select>
              <select name="shows" id="shows" className='bg-transparent select-none outline-none border-0 text-red-500 p-2' onChange={(e)=>{getType(e)}}>
                {shows?.map((d)=>{
                    return <option key={d.id} className='bg-transparent text-red-500' value={d.title}>{d.title}</option>
                  })}
              </select>
    ...
    <DraggableCarousel className='w-full flex lg:flex-col flex-row justify-start items-center lg:overflow-y-scroll lg:overflow-x-hidden overflow-x-scroll overflow-y-hidden scroll-smooth' id="LiveStream">
         ...
         {filterData.length !== 0 ? filterData?.map((d , index)=>{
            return <div key={index} className='w-full h-36 my-1 flex justify-between items-center p-2 select-none'>
              <div className='w-24 h-24 flex justify-center items-center rounded-3xl' >
                 <div className='w-full h-full'>
                 <img src={d.Images[0]? d.Images[1] || d.Images[2] || d.Images[3] || d.Images[4] : d.Images[2] || d.Images[3] || d.Images[4] } alt= {d.Title} className='w-full h-full rounded-3xl' />
                 </div>
              </div>
              <div className='w-1/2 h-full relative text-end flex flex-col justify-start items-start p-2'>
          ...
          }) : <div className='text-red-400 h-36 w-screen flex justify-center items-center text-center text-2xl' > No Results Found </div>}
         </div>
    </DraggableCarousel>
    </>
  );
