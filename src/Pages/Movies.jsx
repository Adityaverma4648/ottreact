...
const Movies = () => {
  ...
  const { loading, upcoming, popular, topRated } = useSelector(
    (state) => state.movie
  );
  ...
  //  fetch useEffects
  useEffect(() => {
    fetchApi("/movie/popular?language=en-US&page=" + page).then((res) => {
      dispatch(getPopular(res));
    });

    fetchApi("/movie/top_rated?language=en-US&page=" + page).then((res) => {
      dispatch(getTopRated(res));
    });

    fetchApi("/movie/upcoming?language=en-US&page=" + page).then((res) => {
